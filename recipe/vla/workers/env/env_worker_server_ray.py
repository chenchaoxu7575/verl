...
A lightweight EnvWorker that uses IsaacSimActorManager (Ray actors)
instead of ZMQ-based IsaacMultiServerClient.

Key differences from EnvWorkerServer:
    1. No ZMQ - uses Ray actor handles directly
    2. Isaac actors managed by IsaacSimActorManager
    3. Data transfer via Ray's object store (more efficient)
    4. Actors can be co-located in same Ray cluster as training
...
Architecture:
    ...
    │                                                                 │
    │  ┌─────────────────────┐      ┌───────────────────────────────┐│
    │  │ EnvWorkerServerRay  │ ───▶ │ IsaacSimActorManager          ││
    │  │   (Ray Worker)      │      │   ├─ Stage 0: [Actor0, ...]   ││
    │  │                     │ ◀─── │   └─ Stage 1: [Actor0, ...]   ││
    │  └─────────────────────┘      └───────────────────────────────┘│
    └─────────────────────────────────────────────────────────────────┘

Usage:
    # In main_ppo.py, import this instead of EnvWorkerServer:
    from recipe.vla.workers.env import EnvWorkerServerRay as EnvWorker

    # Or configure in YAML:
    env:
        train:
            use_ray_actors: true  # Use Ray actors instead of ZMQ
"""
...
from recipe.vla.isaac_server import IsaacSimActorManager
...
def extract_images_and_states(obs):
    ...
    Same as in env_worker_server.py.
    """
    result = {}
...
class EnvWorkerServerRay(Worker):
    """
    EnvWorker that uses Ray-based IsaacSimActorManager.

    This is a drop-in replacement for EnvWorkerServer when using Ray actor mode
    instead of ZMQ server mode.

    Key differences from EnvWorkerServer:
    1. Uses IsaacSimActorManager instead of IsaacMultiServerClient
    2. No ZMQ connections - direct Ray actor calls
    3. Can receive manager from trainer (actors created externally)
    """

    def __init__(self, config: DictConfig, actor_manager: Optional[IsaacSimActorManager] = None):
        """
        Initialize EnvWorkerServerRay.

        Args:
            config: Configuration dict
            actor_manager: Optional pre-created IsaacSimActorManager.
        ...
        self.num_isaac_actors = config.train.get("num_isaac_actors", 8)
        ...
        self._external_manager = actor_manager
        self.manager: IsaacSimActorManager = None
        ...
        # EnvWorkerServerRay is a lightweight coordinator - may not have GPU
        ...
        logger.info(f"[rank={self.rank}] EnvWorkerServerRay initialized")
        logger.info(f"[rank={self.rank}] Using Ray actors (not ZMQ)")
        logger.info(f"[rank={self.rank}] Total envs: {self.total_envs}, stages: {self.stage_num}")

    def set_actor_manager(self, manager: IsaacSimActorManager):
        """
        Set the actor manager (created externally by trainer).
    ...
    def init_worker(self):
        """Initialize the Isaac actor manager.

        If actor_manager was provided in __init__ or set via set_actor_manager(),
        use that. Otherwise, create a new manager.
        """
        if self._external_manager is not None:
            self.manager = self._external_manager
            logger.info(f"[rank={self.rank}] Using external actor manager")
        elif self.manager is not None:
            logger.info(f"[rank={self.rank}] Using pre-set actor manager")
        else:
            # Create new manager
            ...
            logger.info(f"[rank={self.rank}] Creating IsaacSimActorManager...")
            ...
            self.manager = IsaacSimActorManager(
                num_stages=self.stage_num,
                num_actors_per_stage=self.num_isaac_actors,
                num_tasks=num_tasks,
                ...
                # Don't pass runtime_env - let actors inherit sim node's environment
                # Sim nodes are started with python.sh which sets PYTHONPATH, LD_LIBRARY_PATH, etc.
            )

            if not self.manager.initialize():
                raise RuntimeError("Failed to initialize IsaacSimActorManager")

        logger.info(
            f"[rank={self.rank}] Connected to Isaac actors: "
            f"{self.stage_num} stages × {self.num_isaac_actors} actors, "
    ...
    def env_interact_step(self, data: DataProto) -> DataProto:
        ...
        Same interface as EnvWorkerServer but uses Ray actor calls.
        """
    ...
    def _actor_group_step(
        ...
        Uses IsaacSimActorManager.step_batched() for parallel execution.
        """
        # Group by actor_rank
    ...
    def reset_envs_to_state_ids(self, data: DataProto):
        ...
        Uses IsaacSimActorManager for batched reset across all stages.
        """
        ...
        for traj_idx in range(num_trajectories):
            ...
            actor_rank = self.manager.get_actor_for_task(task_id)
