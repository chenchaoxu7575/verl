...
OUTPUT_DIR=${MLP_MODEL_OUTPUT:-"$WORKSPACE/models/vla_libero_grpo"}
VIDEO_OUTPUT=${MLP_MODEL_OUTPUT:-"$WORKSPACE"}/video
SFT_MODEL_PATH=${SFT_MODEL_PATH:-"/workspace/verl_vla/data/Openvla-oft-SFT-libero10-trajall"}
...
PROFILE_STEPS=[4]
PROFILE_RANKS=[0,1,8,9]
PROFILE_DISCRETE=False
# PROFILE_TRACE="'cuda,nvtx,cublas,ucx,vulkan,osrt'" 
PROFILE_TRACE="'cuda,nvtx'"
...
$PYTHON -m recipe.vla.main_ppo \
    ...
    global_profiler.global_tool_config.nsys.discrete=$PROFILE_DISCRETE \
    global_profiler.global_tool_config.nsys.controller_nsight_options.trace=$PROFILE_TRACE \
    global_profiler.global_tool_config.nsys.worker_nsight_options.trace=$PROFILE_TRACE \
    ...
    env.train.profiler.ranks=$PROFILE_RANKS \
    $@ 2>&1 | tee $WORKSPACE/logs/vla_isaac_disagg_w_nsys_profile_$(date +%Y%m%d_%H%M%S).log
